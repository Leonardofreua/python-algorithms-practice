...
def binary_search(list_of_items: List[int], target: int) -> Optional[int]:
  left = 0  # Top of the list.
  right = len(list_of_items) - 1  # Position of the last item on the list.

  while left <= right:  # Keep going while LEFT value (lower) is below or equal to the RIGHT (higher).
...
def binary_search_rec(list_of_items: List[int], target: int, left: int, right: int) -> Optional[int]:
  if left > right:
    return  None
  ...
  if guess < target:  # The guess was TOO LOW.
    ...
    return binary_search_rec(list_of_items, target, left, middle - 1)
